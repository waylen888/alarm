...
```go
spc.Nelson(spc.TrailingRange(50), 50, []spc.Rule{spc.Rule1}) // 單點超出三個 sigma
...
適用於參考期本身可能含有尖峰的情況，`TrailingRange` 取平均與平均移動全距，
適用於參考期會漂移的情況。

後兩者處理的是相反的問題，而且都只處理其中一個。標準差與 MAD 都是全域的離散度量，
所以貫穿參考期的漂移會把它們灌大，反而蓋掉隨後真正的偏移；移動全距是局部的，看不到水位，
但一個離群值會替它貢獻兩個大的全距。在一段「水位持續爬升、雜訊不變」的參考期上，
比參考期末端再高四個單位的偏移，對 `Trailing` 是 2.2σ、對 `TrailingRobust` 是 1.8σ、
對 `TrailingRange` 是 19.2σ；換成一段平穩但含一個大離群值的參考期，三者則是
0.6σ、4.1σ、1.3σ。依你的指標實際會遇到哪一種來選。
...
- **搭配滾動基線的告警大約只持續 `ref` 筆觀測。** 持續偏移會隨著滾動進入自己的參考期，
  ...
  `Fixed` 沒有這個行為。
- **規則是必填參數。** 省略會是編譯錯誤，因為沒有哪一組規則安靜到適合當預設，而一個指標
  該用哪些規則是關於那個指標的判斷。傳入空的 slice、或其中沒有任何本套件認得的規則，
  並不算錯誤：它會裝上 `spc.DefaultRules()`（只有規則 1），因為一個永遠不會成立的條件
  比一個吵的條件更糟。`spc.AllRules()` 是文獻上那套完整程序，搭配 `Trailing(50)` 基線
  在受控製程上每 47 筆誤報一次，只用規則 1 則是每 215 筆；規則 7 是「基線該重估了」的訊號，根本不該進入會呼叫
  人的規則。任何一組規則都不足以讓它單獨勝任 pager——實測表格見套件文件。另外請讓每條規則
  各自成為一個 `alarm.Level`：`Event` 只帶一個浮點數，把多條規則塞進同一個條件就分不出是
  哪一條觸發的，而多個 Level 共用同一個視窗，並且由 `Severity` 告訴你答案。
...
- **核心套件沒有統計型條件**。核心條件都是閾值型與次數型；管制圖型的條件在 [`spc` 子套件](#統計製程管制spc-子套件)，
  再更精細的判斷請自行實作 `Condition`。
