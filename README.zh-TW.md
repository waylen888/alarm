...
- **搭配滾動基線的告警大約只持續 `ref` 筆觀測。** 持續偏移會隨著滾動進入自己的參考期，
  ...
  `Fixed` 沒有這個行為。
- **明確指名要用哪幾條規則，一條規則配一個 `Level`。** 不指名等於八條全開，
  在受控製程上每 47 筆就誤報一次，而只用規則 1 是每 215 筆；規則 7 是
  「基線該重估了」的訊號，不該放進會呼叫人的規則裡。任何一組規則都不足以
  讓它單獨勝任 pager——實測表格見套件文件。請讓每條規則各自成為一個 `alarm.Level`：
  `Event` 只帶一個浮點數，把多條規則塞進同一個條件就分不出是哪一條觸發的，
  而多個 Level 共用同一個視窗，並且由 `Severity` 告訴你答案。
